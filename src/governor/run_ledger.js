...
class RunLedger {
   ...
   */
  startRun({ prompt, model, taskType, agentId, modelPreset } = {}) {
    if (!this._ensureDb()) return null;
    const id = crypto.randomUUID ? crypto.randomUUID().slice(0, 12) : crypto.randomBytes(6).toString('hex');
